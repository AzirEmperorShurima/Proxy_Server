...
const sendHttpsRequest = (hostname, path, method) => {
...
const handleRequest = async (req, res) => {
    ...
    try {
        ...
        if (req.headers.host && req.headers.host.includes('google.com') && req.url.includes('/search')) {
            ...
            logger.info(`Google search query: ${searchQuery}`);

            // Gửi yêu cầu HTTPS
            try {
                const response = await sendHttpsRequest('google.com', '/', 'GET');
                logger.info(`Google response status: ${response.statusCode}`);
                logger.info('Google response headers:', response.headers);
                logger.info('Google response body:', response.body);
            } catch (error) {
